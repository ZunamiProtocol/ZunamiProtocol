...
contract CurveDUSDConvex is Context, Ownable {
    ...
    ICurvePool4 dusdPool;
    IERC20Metadata crv;
    IERC20Metadata cvx;
    IERC20Metadata dusdLP;
    IERC20Metadata dusd;
    ...
    constructor() {
        dusdPool = ICurvePool4(Constants.CRV_DUSD_ADDRESS);
        dusdLP = IERC20Metadata(Constants.CRV_DUSD_LP_ADDRESS);
        crv = IERC20Metadata(Constants.CRV_ADDRESS);
        cvx = IERC20Metadata(Constants.CVX_ADDRESS);
        dusd = IERC20Metadata(Constants.DUSD_ADDRESS);
        ...
        crvRewards = IConvexRewards(Constants.CVX_DUSD_REWARDS_ADDRESS);
        gauge = ICurveGauge(Constants.CRV_DUSD_GAUGE_ADDRESS);
        router = IUniswapRouter(Constants.SUSHI_ROUTER_ADDRESS);
        for (uint256 i = 0; i < POOL_ASSETS; ++i) {
            tokens[i] = dusdPool.underlying_coins(i + 1);
        }
    }
    ...
    modifier onlyZunami() {
        require(
            ...
            "CurvedusdConvex: must be called by Zunami contract"
        );
        _;
    }
    ...
    function getTotalValue() public view virtual returns (uint256) {
        ...
        uint256 lpPrice = dusdPool.get_virtual_price();
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami {
        ...
        uint256 dusdLPs = dusdPool.add_liquidity(amounts4, 0, true);
        dusdLP.safeApprove(Constants.CVX_BOOSTER_ADDRESS, dusdLPs);
        booster.depositAll(Constants.CVX_DUSD_PID, true);
    }
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = dusdPool.calc_token_amount(minAmounts4, false);
        uint256 depositedShare = (crvRewards.balanceOf(address(this)) *
            lpShares) / zunami.totalSupply();
        require(
            depositedShare >= crvRequiredLPs,
            "StrategyCurvedusd: user lps share should be at least required"
        );
        ...
        dusdPool.remove_liquidity(depositedShare, minAmounts4, true);
    ...
    function sellDUSD() public virtual {
        dusd.safeApprove(
            Constants.CRV_DUSD_ADDRESS,
            dusd.balanceOf(address(this))
        );
        dusdPool.exchange(0, 3, dusd.balanceOf(address(this)), 0);
    }
    ...
    function withdrawAll() external virtual onlyZunami {
        ...
        uint256 lpBalance = dusdLP.balanceOf(address(this));
        uint256[4] memory minAmounts;
        dusdPool.remove_liquidity(lpBalance, minAmounts, true);
