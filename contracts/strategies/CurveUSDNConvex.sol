...
contract CurveUSDNConvex is Context, Ownable {
    ...
    ICurvePool4 usdnPool;
    IERC20Metadata crv;
    IERC20Metadata cvx;
    IERC20Metadata usdnLP;
    IERC20Metadata usdn;
    ...
    constructor() {
        usdnPool = ICurvePool4(Constants.CRV_USDN_ADDRESS);
        usdnLP = IERC20Metadata(Constants.CRV_USDN_LP_ADDRESS);
        crv = IERC20Metadata(Constants.CRV_ADDRESS);
        cvx = IERC20Metadata(Constants.CVX_ADDRESS);
        usdn = IERC20Metadata(Constants.USDN_ADDRESS);
        ...
        crvRewards = IConvexRewards(Constants.CVX_USDN_REWARDS_ADDRESS);
        gauge = ICurveGauge(Constants.CRV_USDN_GAUGE_ADDRESS);
        router = IUniswapRouter(Constants.SUSHI_ROUTER_ADDRESS);
        for (uint256 i = 0; i < POOL_ASSETS; ++i) {
            tokens[i] = usdnPool.underlying_coins(i + 1);
        }
    }
    ...
    modifier onlyZunami() {
        require(
            ...
            "CurveusdnConvex: must be called by Zunami contract"
        );
        _;
    }
    ...
    function getTotalValue() public view virtual returns (uint256) {
        ...
        uint256 lpPrice = usdnPool.get_virtual_price();
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami {
        ...
        uint256 usdnLPs = usdnPool.add_liquidity(amounts4, 0, true);
        usdnLP.safeApprove(Constants.CVX_BOOSTER_ADDRESS, usdnLPs);
        booster.depositAll(Constants.CVX_USDN_PID, true);
    }
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = usdnPool.calc_token_amount(minAmounts4, false);
        uint256 depositedShare = (crvRewards.balanceOf(address(this)) *
            lpShares) / zunami.totalSupply();
        require(
            depositedShare >= crvRequiredLPs,
            "StrategyCurveusdn: user lps share should be at least required"
        );
        ...
        usdnPool.remove_liquidity(depositedShare, minAmounts4, true);
    ...
    function sellUSDN() public virtual {
        usdn.safeApprove(
            Constants.CRV_USDN_ADDRESS,
            usdn.balanceOf(address(this))
        );
        usdnPool.exchange(0, 3, usdn.balanceOf(address(this)), 0);
    }
    ...
    function withdrawAll() external virtual onlyZunami {
        ...
        uint256 lpBalance = usdnLP.balanceOf(address(this));
        uint256[4] memory minAmounts;
        usdnPool.remove_liquidity(lpBalance, minAmounts, true);
