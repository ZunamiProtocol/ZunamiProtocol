...
contract CurveFraxConvex is Context, Ownable {
    ...
    constructor() {
        fraxPool = ICurvePool4(Constants.CRV_FRAX_ADDRESS);
        ...
        crvRewards = IConvexRewards(Constants.CVX_FRAX_REWARDS_ADDRESS);
        gauge = ICurveGauge(Constants.CRV_FRAX_GAUGE_ADDRESS);
        router = IUniswapRouter(Constants.SUSHI_ROUTER_ADDRESS);
        for (uint256 i = 0; i < POOL_ASSETS; ++i) {
            tokens[i] = fraxPool.underlying_coins(i + 1);
        }
    }
    ...
    modifier onlyZunami() {
        require(
            ...
            "CurvefraxConvex: must be called by Zunami contract"
        );
        _;
    }
    ...
    function getTotalValue() public view virtual returns (uint256) {
        ...
        uint256 lpPrice = fraxPool.get_virtual_price();
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami {
        ...
        uint256 fraxLPs = fraxPool.add_liquidity(amounts4, 0, true);
        fraxLP.safeApprove(Constants.CVX_BOOSTER_ADDRESS, fraxLPs);
        booster.depositAll(Constants.CVX_FRAX_PID, true);
    }
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = fraxPool.calc_token_amount(minAmounts4, false);
        uint256 depositedShare = (crvRewards.balanceOf(address(this)) *
            lpShares) / zunami.totalSupply();
        require(
            depositedShare >= crvRequiredLPs,
            "StrategyCurvefrax: user lps share should be at least required"
        );
        ...
        fraxPool.remove_liquidity(depositedShare, minAmounts4, true);
    ...
    function sellFrax() public virtual {
        frax.safeApprove(
            Constants.CRV_FRAX_ADDRESS,
            frax.balanceOf(address(this))
        );
        fraxPool.exchange(0, 3, frax.balanceOf(address(this)), 0);
    }
    ...
    function withdrawAll() external virtual onlyZunami {
        ...
        uint256 lpBalance = fraxLP.balanceOf(address(this));
        uint256[4] memory minAmounts;
        fraxPool.remove_liquidity(lpBalance, minAmounts, true);
