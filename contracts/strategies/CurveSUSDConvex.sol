...
contract CurveSUSDConvex is Context, Ownable {
    ...
    ICurvePool4 susdPool;
    IERC20Metadata crv;
    IERC20Metadata cvx;
    IERC20Metadata susdLP;
    IERC20Metadata susd;
    ...
    constructor() {
        susdPool = ICurvePool4(Constants.CRV_SUSD_ADDRESS);
        susdLP = IERC20Metadata(Constants.CRV_SUSD_LP_ADDRESS);
        crv = IERC20Metadata(Constants.CRV_ADDRESS);
        cvx = IERC20Metadata(Constants.CVX_ADDRESS);
        susd = IERC20Metadata(Constants.SUSD_ADDRESS);
        ...
        crvRewards = IConvexRewards(Constants.CVX_SUSD_REWARDS_ADDRESS);
        gauge = ICurveGauge(Constants.CRV_SUSD_GAUGE_ADDRESS);
        router = IUniswapRouter(Constants.SUSHI_ROUTER_ADDRESS);
        for (uint256 i = 0; i < POOL_ASSETS; ++i) {
            tokens[i] = susdPool.underlying_coins(i + 1);
        }
    }
    ...
    modifier onlyZunami() {
        require(
            ...
            "CurvesusdConvex: must be called by Zunami contract"
        );
        _;
    }
    ...
    function getTotalValue() public view virtual returns (uint256) {
        ...
        uint256 lpPrice = susdPool.get_virtual_price();
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami {
        ...
        uint256 susdLPs = susdPool.add_liquidity(amounts4, 0, true);
        susdLP.safeApprove(Constants.CVX_BOOSTER_ADDRESS, susdLPs);
        booster.depositAll(Constants.CVX_SUSD_PID, true);
    }
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = susdPool.calc_token_amount(minAmounts4, false);
        uint256 depositedShare = (crvRewards.balanceOf(address(this)) *
            lpShares) / zunami.totalSupply();
        require(
            depositedShare >= crvRequiredLPs,
            "StrategyCurvesusd: user lps share should be at least required"
        );
        ...
        susdPool.remove_liquidity(depositedShare, minAmounts4, true);
    ...
    function sellSUSD() public virtual {
        susd.safeApprove(address(router), susd.balanceOf(address(this)));

        address[] memory path = new address[](3);
        path[0] = Constants.SUSD_ADDRESS;
        path[1] = Constants.WETH_ADDRESS;
        path[2] = Constants.USDT_ADDRESS;
        router.swapExactTokensForTokens(
    ...
    function withdrawAll() external virtual onlyZunami {
        ...
        uint256 lpBalance = susdLP.balanceOf(address(this));
        uint256[4] memory minAmounts;
        susdPool.remove_liquidity(lpBalance, minAmounts, true);
