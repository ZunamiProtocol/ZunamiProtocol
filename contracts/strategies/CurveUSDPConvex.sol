...
contract CurveUSDPConvex is Context, Ownable {
    ...
    ICurvePool4 usdpPool;
    IERC20Metadata crv;
    IERC20Metadata cvx;
    IERC20Metadata usdpLP;
    IERC20Metadata usdp;
    ...
    constructor() {
        usdpPool = ICurvePool4(Constants.CRV_USDP_ADDRESS);
        usdpLP = IERC20Metadata(Constants.CRV_USDP_LP_ADDRESS);
        crv = IERC20Metadata(Constants.CRV_ADDRESS);
        cvx = IERC20Metadata(Constants.CVX_ADDRESS);
        usdp = IERC20Metadata(Constants.USDP_ADDRESS);
        ...
        crvRewards = IConvexRewards(Constants.CVX_USDP_REWARDS_ADDRESS);
        gauge = ICurveGauge(Constants.CRV_USDP_GAUGE_ADDRESS);
        router = IUniswapRouter(Constants.SUSHI_ROUTER_ADDRESS);
        for (uint256 i = 0; i < POOL_ASSETS; ++i) {
            tokens[i] = usdpPool.underlying_coins(i + 1);
        }
    }
    ...
    modifier onlyZunami() {
        require(
            ...
            "CurveusdpConvex: must be called by Zunami contract"
        );
        _;
    }
    ...
    function getTotalValue() public view virtual returns (uint256) {
        ...
        uint256 lpPrice = usdpPool.get_virtual_price();
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami {
        ...
        uint256 usdpLPs = usdpPool.add_liquidity(amounts4, 0, true);
        usdpLP.safeApprove(Constants.CVX_BOOSTER_ADDRESS, usdpLPs);
        booster.depositAll(Constants.CVX_USDP_PID, true);
    }
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = usdpPool.calc_token_amount(minAmounts4, false);
        uint256 depositedShare = (crvRewards.balanceOf(address(this)) *
            lpShares) / zunami.totalSupply();
        require(
            depositedShare >= crvRequiredLPs,
            "StrategyCurveusdp: user lps share should be at least required"
        );
        ...
        usdpPool.remove_liquidity(depositedShare, minAmounts4, true);
    ...
    function sellUSDP() public virtual {
        usdp.safeApprove(
            Constants.CRV_USDP_ADDRESS,
            usdp.balanceOf(address(this))
        );
        usdpPool.exchange(0, 3, usdp.balanceOf(address(this)), 0);
    }
    ...
    function withdrawAll() external virtual onlyZunami {
        ...
        uint256 lpBalance = usdpLP.balanceOf(address(this));
        uint256[4] memory minAmounts;
        usdpPool.remove_liquidity(lpBalance, minAmounts, true);
