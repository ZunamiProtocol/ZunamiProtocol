...
contract CurveRSVConvex is Context, Ownable {
    ...
    constructor() {
        rsvPool = ICurvePool4(Constants.CRV_RSV_ADDRESS);
        rsvLP = IERC20Metadata(Constants.CRV_RSV_LP_ADDRESS);
        crv = IERC20Metadata(Constants.CRV_ADDRESS);
        cvx = IERC20Metadata(Constants.CVX_ADDRESS);
        rsv = IERC20Metadata(Constants.RSV_ADDRESS);
        ...
        crvRewards = IConvexRewards(Constants.CVX_RSV_REWARDS_ADDRESS);
        gauge = ICurveGauge(Constants.CRV_RSV_GAUGE_ADDRESS);
        router = IUniswapRouter(Constants.SUSHI_ROUTER_ADDRESS);
        for (uint256 i = 0; i < POOL_ASSETS; ++i) {
            tokens[i] = rsvPool.underlying_coins(i + 1);
        }
    }
    ...
    modifier onlyZunami() {
        require(
            ...
            "CurversvConvex: must be called by Zunami contract"
        );
        _;
    }
    ...
    function getTotalValue() public view virtual returns (uint256) {
        ...
        uint256 lpPrice = rsvPool.get_virtual_price();
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami {
        ...
        uint256 rsvLPs = rsvPool.add_liquidity(amounts4, 0, true);
        rsvLP.safeApprove(Constants.CVX_BOOSTER_ADDRESS, rsvLPs);
        booster.depositAll(Constants.CVX_RSV_PID, true);
    }
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = rsvPool.calc_token_amount(minAmounts4, false);
        uint256 depositedShare = (crvRewards.balanceOf(address(this)) *
            lpShares) / zunami.totalSupply();
        require(
            depositedShare >= crvRequiredLPs,
            "StrategyCurversv: user lps share should be at least required"
        );
        ...
        rsvPool.remove_liquidity(depositedShare, minAmounts4, true);
    ...
    function sellRSV() public virtual {
        rsv.safeApprove(
            Constants.CRV_RSV_ADDRESS,
            rsv.balanceOf(address(this))
        );
        rsvPool.exchange(0, 3, rsv.balanceOf(address(this)), 0);
    }
    ...
    function withdrawAll() external virtual onlyZunami {
        ...
        uint256 lpBalance = rsvLP.balanceOf(address(this));
        uint256[4] memory minAmounts;
        rsvPool.remove_liquidity(lpBalance, minAmounts, true);
