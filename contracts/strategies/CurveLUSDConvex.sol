...
contract CurveLUSDConvex is Context, Ownable {
    ...
    ICurvePool4 lusdPool;
    IERC20Metadata crv;
    IERC20Metadata cvx;
    IERC20Metadata lusdLP;
    IERC20Metadata lusd;
    ...
    constructor() {
        lusdPool = ICurvePool4(Constants.CRV_LUSD_ADDRESS);
        lusdLP = IERC20Metadata(Constants.CRV_LUSD_LP_ADDRESS);
        crv = IERC20Metadata(Constants.CRV_ADDRESS);
        cvx = IERC20Metadata(Constants.CVX_ADDRESS);
        lusd = IERC20Metadata(Constants.LUSD_ADDRESS);
        ...
        crvRewards = IConvexRewards(Constants.CVX_LUSD_REWARDS_ADDRESS);
        gauge = ICurveGauge(Constants.CRV_LUSD_GAUGE_ADDRESS);
        router = IUniswapRouter(Constants.SUSHI_ROUTER_ADDRESS);
        for (uint256 i = 0; i < POOL_ASSETS; ++i) {
            tokens[i] = lusdPool.underlying_coins(i + 1);
        }
    }
    ...
    modifier onlyZunami() {
        require(
            ...
            "CurvelusdConvex: must be called by Zunami contract"
        );
        _;
    }
    ...
    function getTotalValue() public view virtual returns (uint256) {
        ...
        uint256 lpPrice = lusdPool.get_virtual_price();
    ...
    function deposit(uint256[3] memory amounts) external virtual onlyZunami {
        ...
        uint256 lusdLPs = lusdPool.add_liquidity(amounts4, 0, true);
        lusdLP.safeApprove(Constants.CVX_BOOSTER_ADDRESS, lusdLPs);
        booster.depositAll(Constants.CVX_LUSD_PID, true);
    }
    ...
    function withdraw(
        ...
        uint256 crvRequiredLPs = lusdPool.calc_token_amount(minAmounts4, false);
        uint256 depositedShare = (crvRewards.balanceOf(address(this)) *
            lpShares) / zunami.totalSupply();
        require(
            depositedShare >= crvRequiredLPs,
            "StrategyCurvelusd: user lps share should be at least required"
        );
        ...
        lusdPool.remove_liquidity(depositedShare, minAmounts4, true);
    ...
    function sellLUSD() public virtual {
        lusd.safeApprove(
            Constants.CRV_LUSD_ADDRESS,
            lusd.balanceOf(address(this))
        );
        lusdPool.exchange(0, 3, lusd.balanceOf(address(this)), 0);
    }
    ...
    function withdrawAll() external virtual onlyZunami {
        ...
        uint256 lpBalance = lusdLP.balanceOf(address(this));
        uint256[4] memory minAmounts;
        lusdPool.remove_liquidity(lpBalance, minAmounts, true);
