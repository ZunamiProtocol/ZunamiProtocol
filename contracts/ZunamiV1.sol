...
contract ZunamiV1 is ERC20, Pausable, AccessControl {
    ...
    event Withdrawn(
        address indexed withdrawer,
        IStrategyV1.WithdrawalType withdrawalType,
        uint256[POOL_ASSETS] tokenAmounts,
        uint256 lpShares,
        uint128 tokenIndex
    );
    ...
    function completeDeposits(address[] memory userList)
        ...
        startedPool
    {
        IStrategyV1 strategy = _poolInfo[defaultDepositPid].strategy;
        ...
        startedPool
    {
        ...
        IStrategyV1 withdrawStrategy = _poolInfo[defaultWithdrawPid].strategy;
            ...
            if (
                !(
                    withdrawStrategy.withdraw(
                        ...
                        IStrategyV1.WithdrawalType.Base,
                        0
                    )
                )
            ) {
        ...
        startedPool
    {
        ...
        IStrategyV1 strategy = _poolInfo[defaultWithdrawPid].strategy;
        ...
        returns (uint256)
    {
        ...
        emit Deposited(_msgSender(), amounts, lpShares, defaultDepositPid);
        return lpShares;
    }
    ...
    function addPool(address _strategyAddr) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        _poolInfo.push(
            PoolInfo({ strategy: IStrategyV1(_strategyAddr), startTime: startTime, lpShares: 0 })
        );
