...
contract ZunamiForwarder is LzApp {
    ...
    event SetGatewayParams(uint256 chainId, address gateway, uint256 tokenPoolId);
    ...
    function _lzReceive(
        ...
        if (messageType == uint8(MessageType.Withdrawal)) {
            require(
                currentWithdrawalId == 0 && currentWithdrawalAmount == 0,
                'Forwarder: previous withdrawal existed'
            );
            ...
            require(
                currentDepositId == 0 && currentDepositAmount == 0,
                'Forwarder: previous deposit existed'
            );

            currentDepositId = messageId;
            currentDepositAmount = convertDecimals(
                tokenAmount,
                tokenDecimals,
                IERC20Metadata(tokens[USDT_TOKEN_ID]).decimals()
            );
    ...
    function delegateCrossDeposit() external onlyRole(OPERATOR_ROLE) {
        require(
            currentDepositId != 0 && currentDepositAmount != 0,
            "Forwarder: deposit wasn't initiated"
        );
        uint256 realDepositAmount = IERC20Metadata(tokens[USDT_TOKEN_ID]).balanceOf(address(this));
        require(
            realDepositAmount >=
                (currentDepositAmount * (SG_SLIPPAGE_DIVIDER - stargateSlippage)) /
                    SG_SLIPPAGE_DIVIDER,
            'Forwarder: not enough provision'
        );

        // delegate deposit to Zunami
        IERC20Metadata(tokens[USDT_TOKEN_ID]).safeIncreaseAllowance(
            address(zunami),
            realDepositAmount
        );
    ...
    function completeCrossDeposit() external payable onlyRole(OPERATOR_ROLE) {
        require(currentDepositId != 0, 'Forwarder: deposit not processing');

        uint256 lpShares = IERC20Metadata(address(zunami)).balanceOf(address(this)) -
            storedLpShares;
    ...
    function completeCrossWithdrawal() external payable onlyRole(OPERATOR_ROLE) {
        // 0/ wait to receive stables from Zunami
        require(
            currentWithdrawalId != 0 && currentWithdrawalAmount != 0,
            'Forwarder: withdrawal is not processing'
        );
        ...
        bytes memory payload = abi.encode(
            ...
            currentWithdrawalId,
            tokenTotalAmount,
            tokens[USDT_TOKEN_ID].decimals()
        );
        _lzSend(gatewayChainId, payload, crossWithdrawalGas);

        storedLpShares -= currentWithdrawalAmount;
        require(
            IERC20Metadata(address(zunami)).balanceOf(address(this)) == storedLpShares,
            "Forwarder: withdrawal wasn't completed in Zunami"
        );
