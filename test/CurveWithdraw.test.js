...
describe('Fetching 3crv price + Checking contract deployment ', () => {
    ...
    describe('Impersonate a 3CRV whale account + transfer the 3CRV + do withdrawal', () => {
        ...
        it(`Whale should transfer ${threeCRVToSendFromWhaleAccount} 3CRV tokens to user`, async function() {
            const owner = (await ethers.getSigners())[0];
            ...
            const transferAmountInWei = ethers.utils.parseEther(String(threeCRVToSendFromWhaleAccount));
        ...
        it(`Contract should be able to partially withdraw $${usdAmountToWithdrawFromCurvePool} in DAI from 3pool curve contract`, async function() {
         ...
         // eslint-disable-next-line max-len
        it(`Contract should be able to partially withdraw $${usdAmountToWithdrawFromCurvePool} in mixed amounts from 3pool curve contract`, async function() {
            const owner = (await ethers.getSigners())[0];
            ...
            const maxLPTokenBurnAmount = parseFloat(usdAmountToWithdrawFromCurvePool) / parseFloat(threecrvUSDPrice);
            ...
            const balanceUSDCWei = ethers.BigNumber.from(await curveContract.methods.balances(1).call()).mul(decimalPadding);
            const balanceUSDTWei = ethers.BigNumber.from(await curveContract.methods.balances(2).call()).mul(decimalPadding);
            ...
            // THIS IS WHERE WE NOW WITHDRAW ALL STABLE COINS IN PROPORTION TO THEIR POOL PERCENTAGES
